...
plt.bar(
    x=pd.to_datetime(["2021-05-06"]).to_pydatetime(),
    height=8,
    width=0.5,
    color="k",
    label="PRP",
...
plt.bar(
    x=pd.to_datetime(["2021-07-06"]).to_pydatetime(),
    height=8,
    width=0.5,
    color="k",
    label="PRP",
...
plt.bar(
    x=pd.to_datetime(["2021-05-06"]).week, height=22, width=0.1, color="k", label="PRP"
)
plt.bar(
    x=pd.to_datetime(["2021-07-06"]).week, height=22, width=0.1, color="k", label="PRP"
)
plt.xlabel("Semana")
plt.legend()
...
plt.plot(
    full.groupby("week").knee_intensity.sum() / 3,
    label="Carga de entrenamiento acumulada",
)
plt.xlabel("Semana")
plt.legend()
